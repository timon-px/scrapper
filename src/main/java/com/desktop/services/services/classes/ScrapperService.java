...
public class ScrapperService implements IScrapperService {
    ...
    @Override
    public CompletableFuture<ScrapperResponseDTO> GetWeb(Path folderPath, String websiteUrl) {
        IStorageWorker storageWorker = new StorageWorker(folderPath);
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                Connection.Response response = Jsoup.connect(websiteUrl).execute();
                ...
                return startProcesses(document, path, filesToSaveList, storageWorker)
                        .thenCompose(unused -> {
                            ...
                            return storageWorker.SaveToFolderAsync(document.outerHtml(), path, ScrapperConstants.HTML_NAME);
                        })
                        .thenApply(finalPath -> {
                            progress.set(1.0);
